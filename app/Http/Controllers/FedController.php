...
class FedController extends Controller
{
    ...
    public function printchildPackage(Request $request){
        ...
        if($type == "indicator"){
            ...
            $query1 = DB::statement("SELECT p.NOMBRE_PROV,p.NOMBRE_DIST,p.FECHA_NACIMIENTO_NINO,p.DOCUMENTO,p.TIPO_SEGURO, P.APELLIDOS_NOMBRES,
                        ...
                        from PAQUETE.dbo.PAQUETE_NINIO_PN p
                            ...
                            LEFT JOIN suple4 S4 on p.DOCUMENTO=S4.Numero_Documento_Paciente
                            LEFT JOIN suple5 S5 on p.DOCUMENTO=S5.Numero_Documento_Paciente
                            LEFT JOIN suple6 S6 on p.DOCUMENTO=S6.Numero_Documento_Paciente
                            LEFT JOIN suple7 S7 on p.DOCUMENTO=S7.Numero_Documento_Paciente
                            LEFT JOIN suple8 S8 on p.DOCUMENTO=S8.Numero_Documento_Paciente
                            LEFT JOIN suple9 S9 on p.DOCUMENTO=S9.Numero_Documento_Paciente
                            LEFT JOIN suplediez suplediez on p.DOCUMENTO=Suplediez.Numero_Documento_Paciente
                            LEFT JOIN suple11 S11 on p.DOCUMENTO=S11.Numero_Documento_Paciente
                            LEFT JOIN tamizaje TMZ on p.DOCUMENTO=TMZ.Numero_Documento_Paciente
                            left join DXANEMIA68 anemia on p.DOCUMENTO=ANEMIA.Numero_Documento_Paciente
            ...
            $query2 = DB::statement("SELECT P.NOMBRE_PROV,p.NOMBRE_DIST,p.FECHA_NACIMIENTO_NINO,p.DOCUMENTO,p.TIPO_SEGURO, P.APELLIDOS_NOMBRES, PR.PREMATURO,
                        ...
                        FROM PAQUETE.DBO.NINO_CRED_SUPLE P
                            LEFT JOIN v90670_1 A1 ON P.DOCUMENTO=A1.Numero_Documento_Paciente
                            LEFT JOIN v90670_2 A2 ON P.DOCUMENTO=A2.Numero_Documento_Paciente
                            LEFT JOIN v90681_1 B1 ON P.DOCUMENTO=B1.Numero_Documento_Paciente
                            LEFT JOIN v90681_2 B2 ON P.DOCUMENTO=B2.Numero_Documento_Paciente
                            LEFT JOIN v90712_13_1 C1 ON P.DOCUMENTO=C1.Numero_Documento_Paciente
                            LEFT JOIN v90712_13_2 C2 ON P.DOCUMENTO=C2.Numero_Documento_Paciente
                            LEFT JOIN v90712_13_3 C3 ON P.DOCUMENTO=C3.Numero_Documento_Paciente
                            LEFT JOIN v90723_1 D1 ON P.DOCUMENTO=D1.Numero_Documento_Paciente
                            LEFT JOIN v90723_2 D2 ON P.DOCUMENTO=D2.Numero_Documento_Paciente
                            LEFT JOIN v90723_3 D3 ON P.DOCUMENTO=D3.Numero_Documento_Paciente
                        LEFT JOIN PREMATUROS PR ON P.DOCUMENTO=PR.Numero_Documento_Paciente");
        ...
        // -----
        if($red_1 == 'TODOS'){
            $nominal = DB::table('CONSOLIDADO_PAQUETE_NINIO_COMPLETO')
                        ->select('*')
                        ->orderBy('NOMBRE_PROV', 'ASC') ->orderBy('NOMBRE_DIST', 'ASC')
                        ->get();
        }
            ...
            $nominal2 = DB::table('CONSOLIDADO_PAQUETE_NINIO_COMPLETO')
                        ->select('*') ->where('NOMBRE_DIST', $dist)
                        ->orderBy('NOMBRE_PROV', 'ASC') ->orderBy('NOMBRE_DIST', 'ASC')
                        ->get();
        }
